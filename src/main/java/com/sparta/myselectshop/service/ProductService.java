...
public class ProductService {
    ...
    public ProductResponseDto updateProduct(Long id, ProductMypriceRequestDto requestDto) {
        ...
        if (myprice < MIN_MY_PRICE) {
            throw new IllegalArgumentException("유효하지 않은 관심 가격입니다." + MIN_MY_PRICE + "원 이상으로 설정해주세요");
        }
    ...
    public Product getPrdocut(Long id) {
    ...
    public List<ProductResponseDto> getAllProducts() {
        ...
        for (Product p : productList) {
            responseDtoList.add(new ProductResponseDto(p));
        }
    ...
    public void addFolder(Long productId, Long folderId, User user) {
        ...
        // 3) 조회한 폴더와 상품이 로그인한 회원의 소유인지 확인합니다.
    ...
    public Page<ProductResponseDto> getProductsInFolter(Long folderId, int page, int size, String sortBy, boolean isAsc, User user) {
        // 페이징 처리
        Sort.Direction direction = isAsc ? Sort.Direction.ASC : Sort.Direction.DESC;
        Sort sort = Sort.by(direction, sortBy);
        Pageable pageable = PageRequest.of(page, size, sort);
